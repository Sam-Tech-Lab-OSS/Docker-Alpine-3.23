# Security Policy — Docker Alpine 3.23

This repository publishes a **minimal, hardened Alpine 3.23 base image** for container use.
...
- official Alpine OCI rootfs as the base source
...
Alpine 3.23 receives security updates for the `main` repository until **1 November 2027**;
`community` is no longer maintained on this branch, Alpine supporting it only until the next
stable release. Monthly rebuilds therefore pick up the security updates Alpine publishes for
3.23, and a fix released upstream reaches the image at the next rebuild.
...
1. Open the repository **[Security tab](https://github.com/Sam-Tech-Lab-OSS/Docker-Alpine-3.23/security)**
...
### Out of scope
- upstream Alpine issues with no available fix yet
...
**1er novembre 2027** ; `community` n'est plus maintenu sur cette branche, Alpine ne le
maintenant que jusqu'à la version stable suivante. Les reconstructions mensuelles récupèrent
donc les mises à jour de sécurité que publie Alpine pour 3.23, et un correctif publié en
amont atteint l'image à la reconstruction suivante.
...
- chaque semaine, le lundi à **01h15 UTC**
...
1. ouvrir l’onglet **[Security](https://github.com/Sam-Tech-Lab-OSS/Docker-Alpine-3.23/security)**
...
### Hors périmètre
- problèmes Alpine amont sans correctif disponible pour le moment
