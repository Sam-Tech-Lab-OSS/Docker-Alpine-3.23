<h1 align="center">Docker Alpine 3.23</h1>
...
<p align="center">
<a href="https://hub.docker.com/r/samtechlab/alpine-3.23"><img src="https://img.shields.io/docker/pulls/samtechlab/alpine-3.23?style=for-the-badge&logo=docker"/></a>
...
A **minimal, hardened, multi-architecture Alpine 3.23 base image**, built
`FROM scratch` from the **official Alpine OCI rootfs** — for authenticity, small size and
reproducibility.

It is designed as a **foundation for your own images**: a complete Alpine system (without a kernel — containers
share the host's), a non-root user,
and a hardened system baseline — then it gets out of your way.

> **Supported architectures:** `linux/amd64`, `linux/arm64`
> **Automatic monthly rebuilds** pick up the security updates Alpine publishes for 3.23.
...
- ✅ **No package cache left behind** — apk runs with `--no-cache`: neither index nor downloaded
  package survives in the image
...
- ✅ **Continuously verified** — hadolint, 10 container integration tests run on **both
...
| Category | Packages |
|---|---|
| Shell & base | `bash`, `coreutils`, `curl`, `findutils`, `jq`, `netcat-openbsd`, `tzdata` |
| System support | `alpine-release`, `ca-certificates`, `shadow` |
...
>   --build-arg PUID=1001 --build-arg PGID=1001 -t my-3.23 .
> ```
>
...
> **Do not install packages at container start** (e.g. an `apk add` in `command:`).
...
Alpine 3.23 receives security updates for the `main` repository until
**1 November 2027**.

The `community` repository is no longer maintained on this branch. Alpine supports
`community` only until the next stable release, and 3.24 shipped in June 2026. A package coming from `community` on this
branch will therefore receive no security updates at all. Check what you depend on with
`apk info -a <package>`, which names the repository a package comes from; if any of it sits
in `community`, use the Alpine 3.24 image instead, supported until 1 June 2028.

Monthly rebuilds pick up the security updates Alpine publishes for 3.23: a fix released
...
- **Images are rebuilt monthly** (1st of the month, 03:00 UTC) against the current 3.23
...
<p align="center">
  <a href="#docker-alpine-323---sam-tech-lab">English version ↑</a> · <b>Français</b>
</p>
...
Une **image de base Alpine 3.23 minimale, durcie et multi-architecture**, construite
`FROM scratch` à partir du **rootfs OCI officiel de Alpine** — pour l'authenticité, la légèreté et
la reproductibilité.

Elle est conçue comme une **fondation pour vos propres images** : un système Alpine complet (sans noyau — les conteneurs
...
- ✅ **Aucun cache de paquets résiduel** — apk tourne avec `--no-cache` : ni index ni paquet
  téléchargé ne subsiste dans l'image
...
- ✅ **Vérifiée en continu** — hadolint, 10 tests d'intégration sur conteneur joués sur **les deux
...
|---|---|
| Shell & base | `bash`, `coreutils`, `curl`, `findutils`, `jq`, `netcat-openbsd`, `tzdata` |
| Outils système | `alpine-release`, `ca-certificates`, `shadow` |
...
> ```bash
> docker buildx build -f Dockerfile-multi-arch \
>   --build-arg PUID=1001 --build-arg PGID=1001 -t mon-3.23 .
> ```
>
...
> **N'installez pas de paquets au démarrage du conteneur** (par ex. un `apk add` dans
...
Alpine 3.23 reçoit des mises à jour de sécurité pour le dépôt `main` jusqu'au
**1er novembre 2027**.

Le dépôt `community` n'est plus maintenu sur cette branche. Alpine ne le maintient que
jusqu'à la version stable suivante, et 3.24 est sortie en juin 2026. Un paquet provenant de `community` sur cette
branche ne recevra donc aucune mise à jour de sécurité. Vérifiez ce dont vous dépendez avec
`apk info -a <paquet>`, qui indique le dépôt d'origine ; si l'un d'eux est dans `community`,
utilisez plutôt l'image Alpine 3.24, supportée jusqu'au 1er juin 2028.

Les reconstructions mensuelles récupèrent les mises à jour de sécurité que Alpine publie
pour 3.23 : un correctif publié en amont atteint cette image à la reconstruction suivante,
...
- **Les images sont reconstruites chaque mois** (le 1er, à 01h15 UTC) à partir de l'archive
  3.23 courante, et peuvent être déclenchées manuellement depuis l'onglet Actions. Voir
  [État du support de l'image de base](#état-du-support-de-limage-de-base) pour ce que cela
  couvre — et ne couvre pas.
- **Les vulnérabilités sont scannées chaque semaine** (lundi, 01h15 UTC) et après chaque build
